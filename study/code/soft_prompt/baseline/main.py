...
def prompt_tuning(model, tokenizer, train_dataset):
    prompt_config = PromptTuningConfig(
        task_type="CAUSAL_LM",
        prompt_tuning_init=PromptTuningInit.TEXT,  
        num_virtual_tokens=20,  
        prompt_tuning_init_text="Here are multiple choice questions about the law. Please choose the appropriate answer.",  # initialize
...
if __name__ =="__main__":
    ...
    print("Device =",device)
    parser  = argparse.ArgumentParser(description="Prompt Tuning Test")
    ...
    parser.add_argument("--prompt_tuning_metric", action=argparse.BooleanOptionalAction, help="To measure the performance of the Prompt Tuned model")
    ...
    if args.info :
        print("\n===== Base Model Info =====")
        get_model_info(model) # To visualize the model structure
    ...
    if args.prompt_tuning_metric:
        print("\n===== Prompt Tuning 진행 =====")
        tuned_model = prompt_tuning(model, tokenizer, train_dataset)

        print("\n===== Fine-tuned Model 성능 측정 =====")
        tuned_model_score = evaluate_model(tuned_model, tokenizer, test_dataset)
        print(f"  - Prompt Tuned Model Accuracy: {tuned_model_score:.4f}")
    
    if args.basemodel_metric and args.prompt_tuning_metric:
        print("\n===== 최종 성능 비교 =====")
        print(f"Base Model Accuracy: {base_model_score:.4f}")
        print(f"Prompt Tuned Model Accuracy: {tuned_model_score:.4f}")
